...
module.exports = class OtgwPlatform extends homebridgeLib.Platform {
  ...
  heartbeat (beat) {
    if (this.client == null) {
      if (this._reconnectBeat == null) {
        this._reconnectBeat = beat + 15
      } else if (beat === this._reconnectBeat) {
        delete this._reconnectBeat
        this.connect()
      }
    }
  }
  ...
  async init () {
    ...
    try {
      ...
      if (Object.keys(this.otgwAccessories).length === 0) {
        ...
        const state = await this.summary()
        for (const service in this.otgwAccessories) {
          this.otgwAccessories[service].setAlive()
          this.otgwAccessories[service].checkState(state)
        }
      }
    } catch (error) {
