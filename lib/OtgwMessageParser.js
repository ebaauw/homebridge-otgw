...
function u8hi (v) { return u8(v.substr(0, 2)) }
function u8lo (v) { return u8(v.substr(2, 2)) }
function s8 (v) {
  v = parseInt(v, 16)
  return (v & 0x80) === 0 ? v : v - 0x100
}
function s8hi (v) { return s8(v.substr(0, 2)) }
function s8lo (v) { return s8(v.substr(2, 2)) }
...
const dataIds = Object.freeze({
  ...
  '0E': [{ key: 'max_reletive_modulation_setting', f: f88 }],
  '0F': [
...
function hex (n) {
  return ('0000' + Number(n).toString(16).toUpperCase()).substr(-4)
}
function twoBytes (v) {
  const a = v.split('/')
...
class OtgwMessageParser extends events.EventEmitter {
  ...
  parseOtMessage (s) {
    ...
    const type = parseInt(a[2].substr(0, 1), 16) & 0x7
    ...
    const id = a[2].substr(2, 2)
    const value = a[2].substr(4, 4)
    const body = {}
    ...
    for (const i in summary) {
      ...
      for (const definition of definitions) {
        body[definition.key] = summary[i].f == null
          ? Number(a[i]) : definition.f(summary[i].f(a[i]))
      }
    }
    return body
  }
...
module.exports = OtgwMessageParser
