...
class OtmClient extends events.EventEmitter {
  constructor (params = {}) {
    super()
    this._host = params.hostname + ':' + params.port
    const options = {
      host: this._host,
      keepAlive: true,
      maxSockets: 1,
    ...
    this._otm = new homebridgeLib.HttpClient(options)
  ...
  async connect () {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._ws.on('message', (message) => {
        this.emit('message', message.slice(17))
      })
      this._ws.on('close', () => {
  ...
  async command (command) {
    for (let retries = 0; retries <= maxRetries; retries++) {
      ...
      const response = await this._otm.get(command)
